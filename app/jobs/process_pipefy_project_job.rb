...
class ProcessPipefyProjectJob < ApplicationJob
  def perform(project_id)
    ...
    project.stages.each do |stage|
      cards_in_stage = PipefyReader.instance.read_phase(stage.integration_id)
  ...
  def process_and_save_cards_in_phase!(project, team, cards_in_stage)
    cards_in_stage.each do |card_id|
      pipefy_response = PipefyApiService.request_card_details(card_id)
      next if pipefy_response.code != 200
      card_response = JSON.parse(pipefy_response.body)
      name_in_pipefy = PipefyReader.instance.read_project_name_from_pipefy_data(card_response['data'])
