...
class ProjectResultsRepository
  ...
  def bugs_closed_until_week(projects, date = Time.zone.today)
    ProjectResult.where(project_id: projects).where('(EXTRACT(WEEK FROM result_date) <= :week AND EXTRACT(YEAR FROM result_date) <= :year) OR (EXTRACT(YEAR FROM result_date) < :year)', week: date.cweek, year: date.cwyear).sum(&:qty_bugs_closed)
  end
