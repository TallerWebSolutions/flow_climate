...
class PortfolioStatisticsData
  ...
  def build_block_by_project_data(portfolio_statistics_chart_adapter)
    ...
    @block_by_project_variation = Stats::StatisticsService.instance.compute_percentage_variation(@block_by_project_data[0].try(:[], :data).min || 0, @block_by_project_data[0].try(:[], :data).max || 0)
  end
  ...
  def build_aging_by_project_data(portfolio_statistics_chart_adapter)
    ...
    @aging_by_project_variation = Stats::StatisticsService.instance.compute_percentage_variation(@aging_by_project_data[0].try(:[], :data).min || 0, @aging_by_project_data[0].try(:[], :data).max || 0)
  end
end
