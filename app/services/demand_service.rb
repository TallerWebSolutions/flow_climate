...
class DemandService
  ...
  def quantitative_consolidation_per_week_to_projects(projects)
    ...
    weeks_array.sort.each do |week_year|
      build_arrival_departure_hash(week_year, projects, arrived_in_week_array, behaviour_data, throughput_in_week_array)
    end
  ...
  def build_arrival_departure_hash(week_year, projects, arrived_in_week_array, behaviour_data, throughput_in_week_array)
    ...
    throughput_week = DemandsRepository.instance.throughput_by_project_and_week(projects, week_year.cweek, week_year.cwyear)
