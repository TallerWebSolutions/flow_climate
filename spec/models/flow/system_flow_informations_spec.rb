...
RSpec.describe Flow::SystemFlowInformations, type: :model do
  ...
  describe '.initialize' do
    context 'having data' do
      ...
      subject(:system_flow_info) { described_class.new(dates_array, Time.zone.today, Demand.all) }
      ...
      it 'assigns the correct information' do
        expect(system_flow_info.dates_array).to eq TimeService.instance.weeks_between_of(Project.all.map(&:start_date).min, Project.all.map(&:end_date).max)
        expect(system_flow_info.demands).to match_array Demand.all
        expect(system_flow_info.current_limit_date).to eq Time.zone.today
      end
    end
