...
RSpec.describe Highchart::OperationalChartsAdapter, type: :data_object do
  context 'having projects' do
    ...
    let(:second_project) { Fabricate :project, customer: customer, status: :waiting, start_date: Time.zone.parse('2018-03-13'), end_date: Time.zone.parse('2018-04-21'), qty_hours: 400, initial_scope: 10 }
    ...
    let!(:first_opened_demand) { Fabricate :demand, project: first_project, created_date: Time.zone.iso8601('2018-02-21T23:01:46-02:00') }
    let!(:second_opened_demand) { Fabricate :demand, project: first_project, created_date: Time.zone.iso8601('2018-02-21T23:01:46-02:00') }

    let!(:first_demand) { Fabricate :demand, project: first_project, downstream: true, created_date: Time.zone.iso8601('2018-01-21T23:01:46-02:00'), end_date: Time.zone.iso8601('2018-02-21T23:01:46-02:00'), leadtime: 2 * 86_400, effort_upstream: 10, effort_downstream: 5 }
    let!(:second_demand) { Fabricate :demand, project: first_project, downstream: true, created_date: Time.zone.iso8601('2018-01-20T23:01:46-02:00'), end_date: Time.zone.iso8601('2018-02-22T23:01:46-02:00'), leadtime: 3 * 86_400, effort_upstream: 12, effort_downstream: 20 }
    let!(:third_demand) { Fabricate :demand, project: second_project, downstream: false, created_date: Time.zone.iso8601('2018-02-18T23:01:46-02:00'), end_date: Time.zone.iso8601('2018-03-19T23:01:46-02:00'), leadtime: 1 * 86_400, effort_upstream: 27, effort_downstream: 40 }
    let!(:fourth_demand) { Fabricate :demand, project: second_project, downstream: false, created_date: Time.zone.iso8601('2018-02-03T23:01:46-02:00'), end_date: Time.zone.iso8601('2018-03-18T23:01:46-02:00'), leadtime: 1 * 86_400, effort_upstream: 80, effort_downstream: 34 }
    let!(:fifth_demand) { Fabricate :demand, project: third_project, downstream: true, created_date: Time.zone.iso8601('2018-01-21T23:01:46-02:00'), end_date: Time.zone.iso8601('2018-03-13T23:01:46-02:00'), leadtime: 4 * 86_400, effort_upstream: 56, effort_downstream: 25 }
    let!(:sixth_demand) { Fabricate :demand, project: first_project, downstream: false, created_date: Time.zone.iso8601('2018-01-15T23:01:46-02:00'), end_date: Time.zone.iso8601('2018-04-30T23:01:46-02:00'), leadtime: 4 * 86_400, effort_upstream: 56, effort_downstream: 25 }

    let!(:first_bug) { Fabricate :demand, project: first_project, demand_type: :bug, created_date: Time.zone.iso8601('2018-01-15T23:01:46-02:00'), end_date: Time.zone.iso8601('2018-04-30T23:01:46-02:00'), leadtime: 4 * 86_400, effort_upstream: 56, effort_downstream: 25 }
    let!(:second_bug) { Fabricate :demand, project: first_project, demand_type: :bug, created_date: Time.zone.iso8601('2018-01-15T23:01:46-02:00'), end_date: Time.zone.iso8601('2018-04-30T23:01:46-02:00'), leadtime: 4 * 86_400, effort_upstream: 56, effort_downstream: 25 }
    let!(:third_bug) { Fabricate :demand, project: first_project, demand_type: :bug, created_date: Time.zone.iso8601('2018-01-15T23:01:46-02:00'), end_date: Time.zone.iso8601('2018-04-30T23:01:46-02:00'), leadtime: 4 * 86_400, effort_upstream: 56, effort_downstream: 25 }
    let!(:fourth_bug) { Fabricate :demand, project: first_project, demand_type: :bug, created_date: Time.zone.iso8601('2018-01-15T23:01:46-02:00'), end_date: Time.zone.iso8601('2018-04-30T23:01:46-02:00'), leadtime: 4 * 86_400, effort_upstream: 56, effort_downstream: 25 }
      ...
      context 'having projects' do
        ...
        context 'and filtering by last month' do
          ...
          it 'do the math and provides the correct information' do
            ...
            expect(report_data.demands_burnup_data.ideal_per_period).to eq [5.5, 11.0]
            expect(report_data.demands_burnup_data.current_per_period).to eq [1, 1]
            expect(report_data.demands_burnup_data.scope_per_period).to eq [11, 11]
            expect(report_data.flow_pressure_data).to eq [0.7142857142857143, 1.0714285714285714]
            ...
            expect(report_data.leadtime_percentiles_on_time[:leadtime_80_confidence]).to eq [345_600.0, 345_600.0]
            ...
            expect(report_data.weeekly_bugs_share_hash).to eq(dates_array: %w[2018-04-30 2018-05-07], bugs_opened_share_array: [0.0, 0.0])
        ...
        context 'and filtering by quarter' do
          ...
          it 'do the math and provides the correct information' do
            ...
            expect(report_data.demands_burnup_data.ideal_per_period).to eq [6.0, 12.0, 18.0, 24.0, 30.0, 36.0, 42.0]
            expect(report_data.demands_burnup_data.current_per_period).to eq [5, 5, 5, 5, 5, 10, 10]
            expect(report_data.demands_burnup_data.scope_per_period).to eq [42, 42, 42, 42, 42, 42, 42]
            expect(report_data.flow_pressure_data).to eq [0.3938649533887629, 0.4548689846308894, 0.5553827418906784, 0.7875688024497548, 0.6618010737058355, 0.5911834344374027, 0.5747558689735561]
            expect(report_data.throughput_per_week).to eq([{ name: I18n.t('projects.charts.throughput_per_week.stage_stream.upstream'), data: [0, 0, 0, 0, 0, 0, 0] }, { name: I18n.t('projects.charts.throughput_per_week.stage_stream.downstream'), data: [0, 0, 0, 0, 0, 5, 0] }])
            expect(report_data.effort_hours_per_month).to eq(data: { downstream: [284.8], upstream: [224.0] }, keys: [[2018.0, 5.0]])
            expect(report_data.lead_time_control_chart[:dispersion_source]).to match_array [[first_bug.demand_id, 4.0], [second_bug.demand_id, 4.0], [third_bug.demand_id, 4.0], [fourth_bug.demand_id, 4.0], [sixth_demand.demand_id, 4.0]]
            expect(report_data.lead_time_control_chart[:percentile_95_data]).to eq 4.0
            expect(report_data.lead_time_control_chart[:percentile_80_data]).to eq 4.0
            expect(report_data.lead_time_control_chart[:percentile_60_data]).to eq 4.0
            ...
            expect(report_data.leadtime_percentiles_on_time[:leadtime_80_confidence]).to eq [276_480.0, 276_480.0, 276_480.0, 276_480.0, 276_480.0, 276_480.0, 345_600.0]
            ...
            expect(report_data.weeekly_bugs_count_hash).to eq(dates_array: %w[2018-03-26 2018-04-02 2018-04-09 2018-04-16 2018-04-23 2018-04-30 2018-05-07], bugs_opened_count_array: [4, 4, 4, 4, 4, 4, 4], bugs_closed_count_array: [0, 0, 0, 0, 0, 4, 4])
            expect(report_data.weeekly_bugs_share_hash).to eq(dates_array: %w[2018-03-26 2018-04-02 2018-04-09 2018-04-16 2018-04-23 2018-04-30 2018-05-07], bugs_opened_share_array: [8.695652173913043, 8.695652173913043, 8.695652173913043, 8.695652173913043, 8.695652173913043, 8.695652173913043, 8.695652173913043])
    ...
    describe '#hours_per_demand_per_week' do
      subject(:report_data) { Highchart::OperationalChartsAdapter.new(Project.all, 'all') }
      it { expect(report_data.hours_per_demand_per_week).to eq [0, 0, 0, 0, 1.5, 0, 0, 0, 0, 0, 0, 0] }
    end
  end
