    ...
    describe 'GET #statistics_tab' do
        ...
        context 'passing valid parameters' do
          context 'no start nor end dates nor period provided' do
            it 'builds the statistic adapter and renders the view using the dates in project to a monthly period' do
              expect(Highchart::ProjectStatisticsChartsAdapter).to receive(:new).with([second_project, first_project], [second_project.start_date, first_project.start_date].min, [second_project.end_date, first_project.end_date].max, 'month').and_call_original
              expect(Highchart::PortfolioChartsAdapter).to receive(:new).with([second_project, first_project], [second_project.start_date, first_project.start_date].min, [second_project.end_date, first_project.end_date].max).and_call_original
          ...
          context 'and a start and end dates provided' do
            it 'builds the statistic adapter and renders the view using the parameters' do
              expect(Highchart::ProjectStatisticsChartsAdapter).to receive(:new).with([second_project, first_project], 1.week.ago.to_date, Time.zone.today, 'month').and_call_original
              expect(Highchart::PortfolioChartsAdapter).to receive(:new).with([second_project, first_project], 1.week.ago.to_date, Time.zone.today).and_call_original
      ...
      context 'having no data' do
        it 'returns empty data set' do
          ...
          expect(assigns(:block_by_project_data)).to eq [{ data: [], marker: { enabled: true }, name: I18n.t('portfolio.charts.block_count') }]
          expect(assigns(:block_by_project_x_axis)).to eq []
        end
      end
    end
  end
