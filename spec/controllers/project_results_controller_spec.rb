    ...
    describe 'POST #create' do
      ...
      context 'passing valid parameters' do
        ...
        it 'assigns the instance variable and renders the template' do
          ...
          expect(result.throughput_upstream).to eq 5
          expect(result.throughput_downstream).to eq 7
      ...
      context 'passing invalid' do
        context 'company' do
          before { post :create, params: { company_id: 'foo', project_id: project, project_result: { result_date: Time.zone.today } } }
          it { expect(response).to have_http_status :not_found }
        end
        context 'project' do
          before { post :create, params: { company_id: company, project_id: 'foo', project_result: { result_date: Time.zone.today } } }
          it { expect(response).to have_http_status :not_found }
        end
        context 'results parameters' do
          ...
          it 'renders the template again showing the errors' do
            expect(response).to render_template :new
            expect(assigns(:project_result).errors.full_messages).to eq ['Time não pode ficar em branco', 'Escopo Conhecido não pode ficar em branco', 'Horas no Upstream não pode ficar em branco', 'Horas no Downstream não pode ficar em branco', 'Horas em Bugs não pode ficar em branco', 'Bugs Fechados não pode ficar em branco', 'Bugs Abertos não pode ficar em branco', 'Data não pode ficar em branco']
          end
        end
      end
    end
    ...
    describe 'PUT #update' do
      ...
      context 'passing valid parameters' do
        ...
        it 'updates the project_result and redirects to projects index' do
          ...
          expect(result.team).to eq team
          expect(result.project).to eq project
          expect(result.result_date).to eq Date.new(2018, 4, 4)
          expect(result.qty_hours_upstream).to eq 10
          expect(result.qty_hours_downstream).to eq 13
          expect(result.throughput_upstream).to eq 5
          expect(result.throughput_downstream).to eq 7
          ...
          expect(result.flow_pressure.to_f).to be_within(0.4).of(6.66)
          expect(result.remaining_days).to eq 3
