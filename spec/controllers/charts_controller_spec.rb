            ...
            context 'no start nor end dates nor period provided' do
              context 'and the project started after 3 months ago' do
                it 'builds the statistic adapter and renders the view using the dates in project to a monthly period' do
                  ...
                  expect(assigns(:end_date)).to eq Date.new(2018, 4, 30)
                  ...
                  expect(assigns(:project_statistics_data).leadtime_data).to match_array [{ data: [0, 7.0], marker: { enabled: true }, name: I18n.t('projects.general.leadtime', percentil: 80) }, { data: [0, 7.0], marker: { enabled: true }, name: I18n.t('projects.general.accumulated_leadtime', percentil: 80) }]
