...
RSpec.describe ProjectResultsRepository, type: :repository do
  ...
  describe '#throughput_in_week_for_projects' do
    context 'when there is data in the week' do
      ...
      it { expect(ProjectResultsRepository.instance.throughput_in_week_for_projects([first_project, second_project])).to eq(1.week.ago.localtime(0).change(hour: 0).beginning_of_week => 30) }
    end
    context 'when there is no data' do
      let!(:project) { Fabricate :project, customer: customer, product: product, start_date: 1.month.ago, end_date: 1.month.from_now }

      it { expect(ProjectResultsRepository.instance.throughput_in_week_for_projects([project])).to eq({}) }
    end
  end
