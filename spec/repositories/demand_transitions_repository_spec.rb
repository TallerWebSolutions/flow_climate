...
RSpec.describe DemandTransitionsRepository, type: :repository do
  describe '#summed_transitions_time_grouped_by_stage_demand_for' do
    context 'having data' do
      ...
      let!(:first_demand) { Fabricate :demand, project: first_project, commitment_date: Time.zone.now, demand_id: 'CAVM-1977' }
      ...
      it { expect(described_class.instance.summed_transitions_time_grouped_by_stage_demand_for(Demand.all)).to eq(first_stage.name => { data: { first_demand.demand_id => 1.day.to_f }, consolidation: 1.day.to_f }, second_stage.name => { data: { second_demand.demand_id => 259_200.0, third_demand.demand_id => nil }, consolidation: 259_200.0 }) }
    end
  ...
  describe '#hours_per_stage' do
    context 'having transitions' do
      ...
      it { expect(described_class.instance.hours_per_stage(Project.all, :upstream, Date.new(2018, 1, 1))).to eq([['fourth_stage', 0, 2_160_000.0]]) }
      it { expect(described_class.instance.hours_per_stage(Project.all, :upstream, Date.new(2018, 2, 2))).to eq([]) }

      it { expect(described_class.instance.hours_per_stage(Project.all, :downstream, Date.new(2018, 1, 1))).to eq([['second_stage', 1, 604_800.0], ['first_stage', 2, 259_200.0], ['fifth_stage', 3, 2_160_000.0], ['third_stage', 4, 3_715_200.0]]) }
      it { expect(described_class.instance.hours_per_stage(Project.all, :downstream, Date.new(2018, 2, 2))).to eq([['second_stage', 1, 604_800.0], ['first_stage', 2, 259_200.0], ['fifth_stage', 3, 2_160_000.0], ['third_stage', 4, 3_715_200.0]]) }
    end

    context 'having no transitions' do
      it { expect(described_class.instance.hours_per_stage(Project.all, :upstream, Date.new(2018, 1, 1))).to eq [] }
    end
  end
end
