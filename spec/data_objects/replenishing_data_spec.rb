...
RSpec.describe ReplenishingData, type: :data_objects do
  ...
  let!(:first_project) { Fabricate :project, customer: customer, team: team, name: 'first_project', status: :executing, start_date: 4.months.ago, end_date: Time.zone.today, max_work_in_progress: 3 }
  let!(:second_project) { Fabricate :project, customer: customer, team: team, name: 'second_project', status: :executing, start_date: 2.months.ago, end_date: 3.days.from_now, max_work_in_progress: 2 }
  let!(:third_project) { Fabricate :project, customer: customer, team: team, name: 'third_project', status: :executing, start_date: 1.month.ago, end_date: 1.week.from_now, max_work_in_progress: 5 }
  ...
  describe '#summary_infos' do
    context 'having data' do
      ...
      it 'returns the hash value' do
        ...
        expect(project_data_to_replenish[0][:montecarlo_80_percent]).to be_within(5).of(54)
        ...
        expect(project_data_to_replenish[0][:customer_happiness]).to be_within(0.005).of(0.019)
        ...
        expect(project_data_to_replenish[1][:relative_flow_pressure]).to be_within(0.9).of(22.5)
        expect(project_data_to_replenish[1][:qty_using_pressure]).to be_within(0.1).of(0.4)
        ...
        expect(project_data_to_replenish[2][:relative_flow_pressure]).to be_within(0.9).of(12.0)
