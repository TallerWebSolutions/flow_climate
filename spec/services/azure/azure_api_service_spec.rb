  ...
  describe '#work_item_updates' do
    ...
    context 'when success' do
      it 'returns the azure response with the work item info' do
        ...
        expect(HTTParty).to(receive(:get).with("#{Figaro.env.azure_base_uri}/#{azure_account.azure_organization}/#{azure_project.project_id}/_apis/wit/workitems/#{work_item_id}/updates?api-version=6.1-preview.3",
                                               basic_auth: { username: azure_account.username, password: azure_account.password },
                                               headers: { 'Content-Type' => 'application/json' })).once

        described_class.new(azure_account).work_item_updates(work_item_id, azure_project.project_id)
      end
    end
    ...
    context 'when failure' do
      ...
      it 'returns an empty hash and logs the error' do
        ...
        expect(described_class.new(azure_account).work_item_updates(1, 'foo')).to eq({})
      end
    end
  end
end
