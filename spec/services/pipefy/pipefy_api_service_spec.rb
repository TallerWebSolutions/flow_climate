...
RSpec.describe Pipefy::PipefyApiService, type: :service do
  ...
  describe '.request_card_details' do
    it 'calls HTTParty' do
      expect(HTTParty).to receive(:post).with(base_uri, body: { query: Pipefy::PipefyProtocol.card_show_request_body(222) }, headers: headers).once
      Pipefy::PipefyApiService.request_card_details('222')
    end
  end
  describe '.request_pipe_details' do
    it 'calls HTTParty' do
      expect(HTTParty).to receive(:post).with(base_uri, body: { query: Pipefy::PipefyProtocol.pipe_show_request_body(222) }, headers: headers).once
      Pipefy::PipefyApiService.request_pipe_details('222')
    end
  end
  describe '.request_cards_to_phase' do
    it 'calls HTTParty' do
      expect(HTTParty).to receive(:post).with(base_uri, body: { query: Pipefy::PipefyProtocol.phase_cards_request_pages(222) }, headers: headers).once
      Pipefy::PipefyApiService.request_cards_to_phase('222')
    end
  end
  describe '.request_next_page_cards_to_phase' do
    it 'calls HTTParty' do
      expect(HTTParty).to receive(:post).with(base_uri, body: { query: Pipefy::PipefyProtocol.phase_cards_paginated(222, 'aswqdf') }, headers: headers).once
