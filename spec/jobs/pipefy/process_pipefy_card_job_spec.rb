...
RSpec.describe Pipefy::ProcessPipefyCardJob, type: :active_job do
  ...
  context 'having params' do
    let(:project) { Fabricate :project, end_date: Time.zone.iso8601('2018-02-20T23:01:46-02:00') }
      ...
      context 'and a pipefy config' do
        ...
        context 'and demand' do
          ...
          it 'updates the demand and the project result' do
            expect(Pipefy::PipefyCardAdapter.instance).to(receive(:create_card!).with(team, card_response).once { demand })
    ...
    context 'and invalid data' do
      context 'and no pipefy config' do
        before do
          stub_request(:post, 'https://app.pipefy.com/queries').with(headers: headers, body: /4648391/).to_return(status: 200, body: blank_card_response.to_json, headers: {})
          stub_request(:post, 'https://app.pipefy.com/queries').with(headers: headers, body: /356528/).to_return(status: 200, body: pipe_response.to_json, headers: {})
        end
        it 'returns doing nothing' do
