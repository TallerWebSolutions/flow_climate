...
RSpec.describe ProcessPipefyCardJob, type: :active_job do
  ...
  context 'having no params' do
    it 'returns doing nothing' do
      expect(PipefyResponseReader.instance).to receive(:create_card!).never
  ...
  context 'having params' do
    ...
    context 'and a pipefy config' do
      ...
      context 'and demand' do
        ...
        it 'updates the demand and the project result' do
          expect(PipefyResponseReader.instance).to(receive(:create_card!).with(project, team, card_response).once { demand })
          expect(PipefyResponseReader.instance).to receive(:update_card!).with(project, team, demand, card_response).once
      ...
      context 'and no demand' do
        it 'updates the demand and the project result' do
          expect(PipefyResponseReader.instance).to receive(:create_card!).with(project, team, card_response).once
          expect(PipefyResponseReader.instance).to receive(:update_card!).never
          ProcessPipefyCardJob.perform_now(params)
        end
      end
    end
    ...
    context 'and no pipefy config' do
      it 'updates the demand and the project result' do
        expect(PipefyResponseReader.instance).to receive(:create_card!).with(project, team, card_response).never
        ProcessPipefyCardJob.perform_now(params)
      end
    end
  end
