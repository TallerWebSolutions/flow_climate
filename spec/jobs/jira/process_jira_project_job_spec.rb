...
RSpec.describe Jira::ProcessJiraProjectJob, type: :active_job do
  ...
  describe '.perform' do
    context 'having params' do
      ...
      let(:jira_project) { client.Project.build(key: 'FC', name: 'Example', projectTypeKey: 'business', lead: 'username') }
        ...
        context 'and a jira config' do
          context 'and demand' do
            it 'calls the adapter to translation' do
              ...
              expect(Jira::JiraIssueAdapter.instance).to receive(:process_issue!).with(jira_account, jira_project, jira_issue).once
