...
RSpec.describe PipefyProtocol, type: :protocol do
  describe '.card_show_request_body' do
    it { expect(PipefyProtocol.card_show_request_body('222')).to eq '{card(id: 222) { id assignees { id } comments { text } fields { name value } phases_history { phase { id } firstTimeIn lastTimeOut } pipe { id } url } }' }
  end
